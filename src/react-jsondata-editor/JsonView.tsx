...
export default function JsonView({
  ...
  input: any;
  jsonPath: string;
  jsonListOutput?: React.RefObject<HTMLDivElement | undefined>;
  deleteNode?: (path: string) => void;
  setPrimitive?: (inputText: string, yoffset: number) => void;
  createModal?: (
    jsonPath: string,
    field: string,
    value: unknown,
  ...
  const changePrimitive = (inputText: string) => {
    if (setPrimitive && jsonListOutput?.current) {
  ...
  const createEditModal = (
    ...
    if (createModal && jsonListOutput?.current) {
      createModal(
        jsonPath,
        field,
        value,
        ...
        inputRef.current.offsetTop +
          ...
          jsonListOutput.current.scrollTop
      );
    }
  };
  ...
  if (input === undefined) {
    ...
    return (
      <div key={indent} className={styles.dataContainer}>
        ...
        >
          <div
            className={styles.needLeaf}
            style={{
              backgroundColor: focusOnLine ? userStyle.themes.hoverColor : "",
            }}
            onMouseOver={() => {
              setFocusOnLine(true);
            }}
    ...
    return (
      <div
        ...
        style={{ color: userStyle.values.null, font: userStyle.values.font }}
      >
    ...
    return (
      <div
        className={styles.valueData}
        style={{ color: userStyle.values.boolean, font: userStyle.values.font }}
      >
        <ValueToString input={input} />
      </div>
    );
    ...
    return (
      <div
        className={styles.valueData}
        style={{ color: userStyle.values.number, font: userStyle.values.font }}
      >
      ...
      {Object.entries(input).map(([key, value]) => {
        return (
          <ViewNode
            ...
            value={value}
            indent={indent}
            isInArray={input instanceof Array}
...
function ViewNode({
  ...
  field: string;
  value: any;
  jsonListOutput: React.RefObject<HTMLDivElement | undefined> | undefined;
  indent: number;
  isInArray: boolean;
  deleteNode: ((path: string) => void) | undefined;
  setPrimitive: ((inputText: string, yoffset: number) => void) | undefined;
  createModal:
    | ((
        jsonPath: string,
        field: string,
  ...
  const [showContent, setShowContent] = useState(
    expandToGeneration === undefined
      ? true
      : (jsonPath.match(/\//g) || []).length <= expandToGeneration
  );
  const [focusOnLine, setFocusOnLine] = useState(false);
  ...
  return (
    <div className={styles.dataContainer}>
      <div className={styles.dataNode} ref={inputRef}>
        <div
          className={styles.clickNode}
          style={{
            backgroundColor: focusOnLine ? userStyle.themes.hoverColor : "",
            cursor: isReadOnly ? "default" : "pointer",
          }}
          onMouseOver={() => {
            setFocusOnLine(!isReadOnly && true);
          }}
          onMouseLeave={() => {
            setFocusOnLine(false);
          }}
        ...
        >
          ...
          >
            ...
            <div
              style={{
                display: "flex",
                color: userStyle.key.color,
                font: userStyle.key.font,
              }}
            >
              <div>
          ...
          {!isReadOnly && focusOnLine && (
            ...
            >
              ...
              >
                <TypeToString input={value} />
              </div>
              <div className={styles.rightButton}>
                <button
                  style={{ backgroundColor: userStyle.buttons.add }}
                  type={"button"}
                  onClick={(e) => {
                    e.stopPropagation();
                    e.preventDefault();
          ...
          {!isList && (
            <JsonView
              jsonPath={jsonPath /* TODO IS THIS CORRECT */}
              input={value}
              needLeaf={false}
              expandToGeneration={expandToGeneration}
              isReadOnly={isReadOnly}
