  ...
  function saveJsonObject() {
    if (valueType === "array") {
      if (path === "/") {
        changeNode(
          "/" + field,
          DeepCopy(pointer.get(modalObj, "/" + field + "/" + value.length))
        );
      } else
        changeNode(
          path + "/" + field + "/" + value.length,
          DeepCopy(pointer.get(modalObj, "/" + field + "/" + value.length))
        );
    } else if (valueType === "object") {
      Object.keys(pointer.get(modalObj, "/" + field)).forEach((key) => {
        changeNode(
          path + "/" + field + "/" + key,
          DeepCopy(pointer.get(modalObj, "/" + field + "/" + key))
        );
      });
    } else {
        ...
        <div className={styles.modalBtnContainer}>
          {valueType === "object" ? (
            ...
            <button
              ...
              style={{ backgroundColor: userStyle.buttons.delete }}
              onClick={() => {
          ...
          <div className={styles.modalModifyBtnContainer}>
            <button
              type={"button"}
              className={styles.modalButton}
              style={{ backgroundColor: userStyle.buttons.cancel }}
              onClick={() => {
            ...
            <button
              type={"submit"}
              className={styles.modalButton}
              style={{ backgroundColor: userStyle.buttons.update }}
            >
              <span
                style={{ font: userStyle.values.font, lineHeight: "normal" }}
              >
...
function setDefaultValue(type: string) {
  if (type === "null") {
    return null;
...
function NullEditor({
  ...
  function changeField(inputField: string) {
      ...
      <div>
        {!isInArray ? (
          ...
          <div className={styles.arrayIndex}>
            <div className={styles.arrayIndexInput}>{inputField}</div>
          </div>
        )}
      </div>
...
function BooleanEditor({
  ...
  const [inputValue, setInputValue] = useState(
    pointer.get(modalObj, path + "/" + field) !== false
  );

  function saveValue(newValue: any) {
  ...
  // when there is same field, returns previous field
  function changeField(inputField: string) {
    if (saveField(inputField, inputValue, field)) setInputField(field);
  }
...
function NumberEditor({
  ...
  function changeValue(inputValue: string) {
  ...
  function valueOnBlur(inputValue: string) {
    const numberValue = Number(inputValue);
  ...
  function AddNode() {
    pointer.set(
      ...
      path +
        ...
        arrayField +
        "/" +
        pointer.get(modalObj, path + "/" + arrayField).length,
      ""
    );
    setRender(!render);
  }
  ...
  return (
    <div>
      <div className={styles.modalContentContainer}>
        <div>
          {!isInArray ? (
            ...
            <div className={styles.arrayIndex}>
              <div className={styles.arrayIndexInput}>{arrayField}</div>
            </div>
          )}
        </div>
        <div className={styles.modalNoLabel}>
        ...
        <div className={styles.modalNoLabel}>
          {!(path === "" || isRoot || rerender === undefined) && (
            <div className={styles.delButton}>
              <i className={styles.removeIcon} onClick={removeList} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className={styles.addLine}>
        ...
        <div>
          {Object.entries(pointer.get(modalObj, path + "/" + field)).map(
            ([key, value]) => {
              return (
                <div
          ...
          <div className={styles.nodePlusLineContainer}>
            ...
            <div className={styles.addButton}>
              <i className={styles.addIcon} onClick={AddNode} />
            </div>
          </div>
        </div>
      </div>
...
function ObjectEditor({
  ...
  // change its filed
  function changeField(inputValue: string) {
    if (!pointer.has(modalObj, path + "/" + inputValue)) {
      pointer.set(
        modalObj,
        path + "/" + inputValue,
        DeepCopy(pointer.get(modalObj, path + "/" + ObjField))
      );
  ...
  return (
    <div>
      <div className={styles.modalContentContainer}>
        <div>
          {!isInArray ? (
            ...
            <div className={styles.arrayIndex}>
              <div className={styles.arrayIndexInput}>{ObjField}</div>
            </div>
          )}
        </div>
        <div className={styles.modalNoLabel}>
      ...
      <div className={styles.addLine}>
        ...
        <div>
          <div>
            {Object.entries(pointer.get(modalObj, path + "/" + ObjField)).map(
              ([key, value]) => {
                return (
                  <div
                    className={styles.listContainer}
          ...
          <div className={styles.nodePlusLineContainer}>
            ...
            {hasEmptyField && (
              <div className={styles.warning}>
                <span>please insert a key value</span>
              </div>
            )}
          </div>
        </div>
